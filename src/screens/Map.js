...
const routesData = routesDataFinder(new Date().getHours());
...
class Map extends React.Component {
  ...
  downloadRegions = async () => {
    alert("BASLADI");
    this.setState({ loading: true });
    const tiles = await this.tileGridForRegion(this.state.Region, 11, 18);
  ...
  tileGridForRegion = (region, minZoom, maxZoom) => {
    let tiles = [];
    for (let zoom = minZoom; zoom <= maxZoom; zoom++) {
      const subTiles = this.tilesForZoom(region, zoom);
  ...
  latToTileY = (lat, zoom) => {
    return Math.floor(
      ((1 -
        Math.log(
          Math.tan(this.degToRad(lat)) + 1 / Math.cos(this.degToRad(lat))
        ) /
          Math.PI) /
        2) *
        Math.pow(2, zoom)
  ...
  fetchTiles = async (tileGrid, rootFolder, tileServerUrl) => {
    ...
    let counter = 0;
    await tileGrid.forEach(async tile => {
      const fetchUrl = `${tileServerUrl}/${tile.z}/${tile.x}/${tile.y}.png`;
      const fsLocation = `${rootFolder}/${tile.z}/${tile.x}/${tile.y}.png`;
      try {
        await FileSystem.downloadAsync(fetchUrl, fsLocation);
        console.log(counter++ / tileGrid.length);
      } catch (e) {
        console.log(e);
      }
    });
  ...
  render() {
    ...
    return (
      <Container>
        ...
        >
          ...
          >
            ...
            {this.state.locationMarker && (
              <Marker coordinate={this.state.locationMarker}>
                <Icon
                  name="location-arrow"
                  type="FontAwesome5"
                  style={{ fontSize: 20, color: "#3493eb" }}
                />
              </Marker>
            )}
        ...
        <TouchableOpacity style={{ height: 50 }} onPress={this.downloadRegions}>
          <Text>{!this.state.loading ? "Download!" : "Yukleniyor"}</Text>
        </TouchableOpacity>
        <View style={{ flex: 1 }}>
          <RouteInfo
